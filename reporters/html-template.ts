...
export function buildHtml({ results, options }: BuildHtmlParams): string {
  const title = options.title ?? 'Playwright LLM Report';

  const llmEndpoint = options.llmEndpoint
    || (typeof options.llmPort === 'number'
      ? `http://localhost:${options.llmPort}/llm/analyze`
  ...
  <script>
    ...
    function buildMetricsRow(metrics) {
      ...
      extra.textContent =
        ...
        '% · Avg duration: ' + msToHuman(avgDuration) +
